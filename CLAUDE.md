...
- **処理モード**:
  ...
  - 黒塗り処理（情報隠蔽）
  - 白塗り処理（不透明な白での塗りつぶし）
  - ブラー処理（ぼかし。デコンボリューションによる復元可能性があるため秘匿用途にはモザイク/黒塗りを推奨）
- **範囲選択**: マウス/タッチのドラッグ、またはキーボード（矢印キー移動・Shift+矢印リサイズ・Enter適用・Escape解除）
...
```
├── components/
│   └── ImageMosaic.vue     # オーケストレーター（composablesの結線とテンプレート）
├── composables/            # ロジック本体（テストはここを直接importする）
│   ├── useImageUpload.ts   # ファイル入出力・DnD・クリップボード・エラー表示
│   ├── useCanvasRenderer.ts # canvas描画・座標系・オーバーレイ・ResizeObserver
│   ├── useAreaSelection.ts # 範囲選択（ドラッグ+キーボード操作）
│   ├── useUndoHistory.ts   # 領域差分方式のアンドゥスタック
│   └── useImageProcessing.ts # 黒塗り/白塗り/モザイク/ブラー処理
├── utils/
│   └── downloadFilename.ts # ファイル名生成（純関数）
├── test/                   # テストスイート
│   ├── coordinate-calculation.test.ts
│   ├── state-management.test.ts
│   ├── undo-functionality.test.ts
│   ├── download-functionality.test.ts
│   ├── keyboard-selection.test.ts
│   └── setup.ts
├── public/
│   └── _headers            # Cloudflare Pages用セキュリティヘッダー
├── app.vue                 # ルートコンポーネント
├── nuxt.config.ts         # Nuxt設定（Cloudflare Pages用）
├── vitest.config.ts       # テスト設定（カバレッジしきい値含む）
